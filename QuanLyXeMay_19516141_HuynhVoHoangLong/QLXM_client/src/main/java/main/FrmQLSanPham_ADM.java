...
public class FrmQLSanPham_ADM extends JFrame implements ActionListener, MouseListener {
	/**
	 * 
	 */
	...
	public FrmQLSanPham_ADM() {
		...
		pnThongtin.setBorder(BorderFactory.createTitledBorder(BorderFactory.createTitledBorder(
				BorderFactory.createLineBorder(Color.RED, 2), "Thông tin sản phẩm",
...
//		new FrmQLSanPham_ADM().setVisible(true);
//
//	}
		...
		if (o.equals(btnXoaTrang)) {
			...
			if (txtmaXe.getText().equals("") || txttenXe.getText().equals("") || txtloaiXe.getText().equals("")
				...
				if (timmaxe(sp.getMaXe())) {
					JOptionPane.showMessageDialog(this, "Mã sản phẩm đã tồn tại");
				} else
					try {
						sp_DAO.create(sp);
			...
			String temp = "";
			if (ma.equals("")) {
				JOptionPane.showMessageDialog(this, "Cần nhập mã sản phẩm cần tìm");
			} else if (timmaxe(ma)) {
				int row = table.getSelectedRow();
				txtmaXe.setText(model.getValueAt(row, 0).toString());
				txttenXe.setText(model.getValueAt(row, 1).toString());
				...
				JOptionPane.showMessageDialog(this, "Không tìm thấy mã sản phẩm cần tìm");
			}
	...
	private boolean ktdulieu() {
		...
		if (!(maxe.length() > 0 && maxe.matches("^[A-Z]{2,}+[0-9]{1,}"))) {
			JOptionPane.showMessageDialog(this,
					"Mã xe bắt đầu bằng hai kí tự hoa tiếp theo là các kí tự số");
			txtmaXe.selectAll();
			txtmaXe.requestFocus();
			return false;
		}
